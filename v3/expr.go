...
type expr struct {
	...
	auxBits uint16
	// The input vars bitmap specified required input variables that are not
	// otherwise provided by inputs. For scalar operators, this will by the set
	// of all variables referenced by the scalar expression. For relational
	// operators, this is the set of variables needed by the operator that are
	// not provided by the inputs. The only reason a relational operator will
	// have input variables that are not provided by its inputs are correlated
	// subqueries. The indexes refer to queryState.columns which is constructed
	// on a per-query basis by the columns required by filters, join conditions,
	// and projections and the new columns generated by projections.
	inputVars bitmap
	children  []*expr
	props     *logicalProps
