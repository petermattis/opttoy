...
func (e *expr) columns() []bitmapIndex {
	// TODO(peter): this isn't correct. Only projectOp contains projections. For
	// selectOp, the output columns flow through from the input columns. For join
	// op, the output columns depend on the type of join.
...
func updateRelationProperties(expr *expr) {
	// TODO(peter): these tests on expr.op are ugly. We likely need different
	// property update functions per operator.
	if expr.op != scanOp {
		expr.inputVars = 0
