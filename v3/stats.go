...
// initHistogramWalker returns a new histogramWalker for histogram
// h, initialized at the first bucket.
func initHistogramWalker(h *histogram) *histogramWalker {
	if len(h.buckets) == 0 {
		return &histogramWalker{done: true}
	}

	buc := h.buckets[0]
	return &histogramWalker{
		h:    h,
		idx:  0,
		b:    buc,
		lb:   h.getLowerBound(),
...
// getOverlappingBuckets finds the overlapping buckets of the histogramWalkers
...
//       |___||____|__|___|__|
//
// The function assumes that both histogramWalkers initially point to buckets
// which have the same lower bound.
func (w *histogramWalker) getOverlappingBuckets(
	other *histogramWalker,
	mergeBuckets func(bucket, bucket, int64) bucket,
) []bucket {
	var buckets []bucket
	splitAndMergeBuckets := func(wA, wB *histogramWalker) {
		var newBuc bucket
...
func (h *histogram) orHistograms(other *histogram) *histogram {
	var buckets []bucket
	wThis := initHistogramWalker(h)
	wOther := initHistogramWalker(other)

	// If one histogram has lower buckets than the other, add those buckets first.
	addLeadingBuckets := func(wA, wB *histogramWalker) {
	...
	addTrailingBuckets := func(w *histogramWalker) {
		for !w.done {
...
func (h *histogram) andHistograms(other *histogram) *histogram {
	...
	skipLeadingBuckets := func(wA, wB *histogramWalker) {
		for wA.lb < wB.lb && !wA.done {
			if wB.lb < wA.ub {
