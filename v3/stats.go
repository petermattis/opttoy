...
func (h *histogram) String() string {
	...
	for i, b := range h.buckets {
		...
		fmt.Fprintf(&buf, "%s:%d", b.lowerBound, b.count)
	}
	return buf.String()
}

// Create a histogram from an INSERT clause. The rows are expected to be a
// VALUES clause containing pairs of (lower-bound, count). Currently only INT
// or NULL constants are valid for lower-bound, and only INT constants are
// valid for count.
func createHistogram(
	...
	for i, v := range values.Tuples {
		...
		if v.Exprs[0] == tree.DNull {
			col.hist.buckets[i].lowerBound = tree.DNull
			continue
		}

		switch t := v.Exprs[0].(type) {
		case *tree.NumVal:
			col.hist.buckets[i].lowerBound, err = t.ResolveAsType(nil, types.Int)
			if err != nil {
				fatalf("malformed histogram bucket: %s: %v", v, err)
			}
	...
	sort.Slice(col.hist.buckets, func(i, j int) bool {
		...
		return bi.lowerBound.Compare(nil, bj.lowerBound) < 0
	})
